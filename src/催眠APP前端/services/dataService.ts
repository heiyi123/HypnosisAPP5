...
const STORE_SCHEMA: z.ZodType<PersistedStore> = z
  .object({
    ...
    nextCustomQuestId: z.coerce.number().optional(),
...
export const DataService = {
  ...
  createCustomQuest: async ({
  ...
  }): Promise<{ success: boolean; user?: UserResources }> => {
    const trimmedName = name.trim();
    const trimmedDesc = description.trim();
    if (!trimmedName || !trimmedDesc) return { success: false };
    if (trimmedName.includes('.')) return { success: false };

    const reward = Math.max(1, Math.min(9999, Math.floor(rewardMcPoints)));
    const costMoney = reward * 800;

    const user = await DataService.getUserData();
    if (user.money < costMoney) return { success: false };

    const { store } = normalizeChatVariables(getVariables(CHAT_OPTION));
    const nextId =
      typeof store.nextCustomQuestId === 'number' && Number.isFinite(store.nextCustomQuestId)
    ...
    const nextUser = await DataService.updateResources({ money: user.money - costMoney });
