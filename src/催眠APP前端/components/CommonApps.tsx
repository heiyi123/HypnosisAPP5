...
export const RoleEntryApp = ({ onBack }: { onBack: () => void }) => {
...
${roleName}现在的性欲:
<%_ if (getvar('stat_data.角色.${roleName}.发情值') < 20) { _%>
- 性欲不高
<%_ } else if (getvar('stat_data.角色.${roleName}.发情值') < 40) { _%>
- 稍微有些性唤起，但整体正常
<%_ } else if (getvar('stat_data.角色.${roleName}.发情值') < 60) { _%>
- 感觉到有些性饥渴，可以感觉到自己性欲来了，但不会失去控制
<%_ } else if (getvar('stat_data.角色.${roleName}.发情值') < 80) { _%>
- 感到性饥渴，性欲明显，稍微会影响判断，但不会被性欲控制
<%_ } else if (getvar('stat_data.角色.${roleName}.发情值') < 95) { _%>
- 角色感觉到非常性饥渴，性欲开始影响判断，但还不会完全左右判断，还保留基本理智
<%_ } else if (getvar('stat_data.角色.${roleName}.发情值') >= 95) { _%>
- 角色已经满脑子都是色色，判断被性欲影响，一切以满足性欲优先
<%_ } _%>
${roleName}对<user>的态度:
  associated variable: 警戒度({{get_message_variable::stat_data.角色.${roleName}.警戒度}})
  <%_ if (getvar('stat_data.角色.${roleName}.警戒度') < 20) { _%>
    行为指导:
${g[0] ? pad(g[0]) : '      -'}
  <%_ } else if (getvar('stat_data.角色.${roleName}.警戒度') < 40) { _%>
    行为指导:
${g[1] ? pad(g[1]) : '      -'}
  <%_ } else if (getvar('stat_data.角色.${roleName}.警戒度') < 60) { _%>
    行为指导:
${g[2] ? pad(g[2]) : '      -'}
  <%_ } else if (getvar('stat_data.角色.${roleName}.警戒度') < 80) { _%>
    行为指导:
${g[3] ? pad(g[3]) : '      -'}
  <%_ } else { _%>
    行为指导:
${g[4] ? pad(g[4]) : '      -'}
  <%_ } _%>
${roleName}对{{user}}的好感度: {{get_message_variable::stat_data.角色.${roleName}.好感度}}
  <%_ if (getvar('stat_data.角色.${roleName}.好感度') < 20) { _%>
    行为指导:
${l[0] ? pad(l[0]) : '      -'}
  <%_ } else if (getvar('stat_data.角色.${roleName}.好感度') < 40) { _%>
    行为指导:
${l[1] ? pad(l[1]) : '      -'}
  <%_ } else if (getvar('stat_data.角色.${roleName}.好感度') < 60) { _%>
    行为指导:
${l[2] ? pad(l[2]) : '      -'}
  <%_ } else if (getvar('stat_data.角色.${roleName}.好感度') < 80) { _%>
    行为指导:
${l[3] ? pad(l[3]) : '      -'}
  <%_ } else { _%>
    行为指导:
${l[4] ? pad(l[4]) : '      -'}
  <%_ } _%>
${roleName}服从度: {{get_message_variable::stat_data.角色.${roleName}.服从度}}
  <%_ if (getvar('stat_data.角色.${roleName}.服从度') < 20) { _%>
    行为指导:
${o[0] ? pad(o[0]) : '      -'}
  <%_ } else if (getvar('stat_data.角色.${roleName}.服从度') < 40) { _%>
    行为指导:
${o[1] ? pad(o[1]) : '      -'}
  <%_ } else if (getvar('stat_data.角色.${roleName}.服从度') < 60) { _%>
    行为指导:
${o[2] ? pad(o[2]) : '      -'}
  <%_ } else if (getvar('stat_data.角色.${roleName}.服从度') < 80) { _%>
    行为指导:
${o[3] ? pad(o[3]) : '      -'}
  <%_ } else { _%>
    行为指导:
${o[4] ? pad(o[4]) : '      -'}
  <%_ } _%>
rule:
  - 行为指导优先于作为背景的角色关键信息和角色详情
  - 好感度和服从度行为可以混合
  - 角色的好感与服从度要优先于警戒度, 只要好感度或服从度大于警戒度, 就不会触发警戒
</${roleName}行为指导>`;

    const personaTxt = `${personaBlock}
${behaviorTxt}
`;
  ...
  return (
    <PageLayout title="角色录入" onBack={onBack}>
...
export const PublishTaskApp = ({
  ...
  const reward = (() => {
    const n = Number.parseInt(rewardInput, 10);
  ...
  return (
    <PageLayout title="发布任务" onBack={onBack}>
      <div className="space-y-4">
        <div className="text-xs text-gray-500">
          当前资金：¥{userData.money.toLocaleString()}，当前点数：{userData.mcPoints} PT
        </div>

        <div className="space-y-2">
          <div className="text-sm font-medium text-gray-800">任务名称</div>
          <input
            value={title}
        ...
        <div className="space-y-2">
          <div className="text-sm font-medium text-gray-800">任务内容</div>
          <textarea
            value={content}
            onChange={e => setContent(e.target.value)}
            rows={4}
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-gray-500 resize-none"
          />
        </div>

        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="text-sm font-medium text-gray-800">任务报酬（PT）</div>
            <div className="text-xs text-gray-500">每 1 PT 消耗 800 零花钱</div>
          </div>
          <input
            type="number"
            inputMode="numeric"
            ...
            onBlur={() => setRewardInput(reward > 0 ? String(reward) : '1')}
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-gray-500"
          />
          <div className="text-xs text-gray-600">
            预计花费：¥{costMoney.toLocaleString()}
            {userData.money < costMoney && <span className="text-red-500 ml-2">余额不足</span>}
          </div>
        </div>
